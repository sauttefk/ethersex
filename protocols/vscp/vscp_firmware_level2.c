...
void
vscp_readRegister(vscpEvent * pEvent, vscpEvent * pOutEvent)
{

uint32_t i;

uint32_t idx = ((uint32_t) pEvent->data[0] << 24) +
    ((uint32_t) pEvent->data[1] << 16) +
    ((uint32_t) pEvent->data[2] << 8) +
(uint32_t) pEvent->data[3];


uint32_t cnt = ((uint32_t) pEvent->data[4] << 8) +
    (uint32_t) pEvent->data[5];
...
if ((idx + i) > VSCP_LEVEL2_COMMON_REGISTER_START)
    ...
    else
    {
        ...
        // User register
        pOutEvent->data[8 + i] = vscp_readAppReg(idx + i);
...
pOutEvent->sizeData = 8 + cnt;

pOutEvent->data[0] = pEvent->data[0];

pOutEvent->data[1] = pEvent->data[1];

pOutEvent->data[2] = pEvent->data[2];

pOutEvent->data[3] = pEvent->data[3];
...
pOutEvent->head = (VSCP_PRIORITY_HIGH << 5);

pOutEvent->vscp_class = VSCP_CLASS2_PROTOCOL;

pOutEvent->vscp_type = VSCP2_TYPE_PROTOCOL_READ_REGISTER;


vscp_sendEvent(pOutEvent);
...
void
vscp_writeRegister(vscpEvent * pEvent, vscpEvent * pOutEvent)
{
...
uint16_t cnt = pEvent->sizeData - 16 - 4 - 4;      // Take away GUID + addr + reserved
...
if ((idx + i) > VSCP_LEVEL2_COMMON_REGISTER_START)
    {
        ...
        // Common register
        pOutEvent->data[8 + i] =
        vscp_writeStdReg((idx & 0xff) + i, pEvent->data[24 + i]);
    ...
    else
    {
        ...
        // User register
        pOutEvent->data[8 + i] =
        vscp_writeAppReg(idx + i, pEvent->data[24 + i]);
...
pOutEvent->vscp_type = VSCP2_TYPE_PROTOCOL_WRITE_REGISTER;
...
rv = (vscp_page_select & 0xff);
             ...
             (vscp_readStdReg < VSCP_REG_NICKNAME_ID))
  {
    ...
    else
    {
        ...
        // Write
        vscp_setManufacturerId(vscp_readStdReg - VSCP_REG_MANUFACTUR_ID0,
                               data);
